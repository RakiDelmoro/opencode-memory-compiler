...
def check_broken_links() -> list[dict]:
    ...
    for article in list_wiki_articles():
        ...
        for link in extract_wikilinks(content):
            if link.startswith("daily/") or link.startswith("logs/"):
                continue
...
def check_missing_backlinks() -> list[dict]:
    """Check for asymmetric links: A links to B but B doesn't link back to A."""
    issues = []
    for article in list_wiki_articles():
        content = article.read_text(encoding="utf-8")
        rel = article.relative_to(KNOWLEDGE_DIR)
...
Do NOT output anything else — no preamble, no explanation, just the formatted lines."""
    ...
    try:
        ...
        return [{"severity": "error", "check": "contradiction", "file": "(system)", "detail": "LLM check timed out"}]
...
def generate_report(all_issues: list[dict]) -> str:
    ...
    for severity, issues_list, marker in [
        ("Errors", errors, "x"),
        ("Warnings", warnings, "!"),
